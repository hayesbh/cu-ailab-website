...
export function ProjectCard({ project, showLink = true }: ProjectCardProps) {
  return (
    <article className="group relative h-[400px] bg-white dark:bg-[#32311b] rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-[#e5e5dc] dark:border-[#3a3928]">
          ...
          <div className="flex gap-2 mb-3 opacity-100 group-hover:opacity-100 transition-opacity delay-100">
            {project.categories ? (
              project.categories.slice(0, 3).map((cat, idx) => (
                <span key={idx} className="inline-flex items-center justify-center h-8 leading-tight px-2.5 py-1 bg-primary text-text-main text-[8px] text-center font-bold uppercase tracking-wider rounded-md">
                  {cat}
                </span>
              ))
            ) : project.category ? (
              <span className="inline-flex items-center justify-center h-8 leading-tight px-2.5 py-1 bg-primary text-text-main text-[8px] text-center font-bold uppercase tracking-wider rounded-md">
                {project.category}
              </span>
            ) : null}
      ...
      <div className="h-0 group-hover:h-auto overflow-hidden opacity-0 group-hover:opacity-100 transition-all duration-300 ease-in-out">
            ...
            {showLink && project.external_url && project.external_url !== '#' && (
              <Link 
